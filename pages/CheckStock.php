...
// Persist pilihan zone/lokasi supaya tidak reset saat tombol Check diklik
if (isset($_POST['zone'])) {
	$_SESSION['checkstock_zone'] = strtoupper(trim($_POST['zone']));
}
if (isset($_POST['lokasi'])) {
	$_SESSION['checkstock_lokasi'] = strtoupper(trim($_POST['lokasi']));
}
...
if ($sqlCek1 === false) {
	$sql_errors[] = "Gagal ambil count stokfull status ok untuk zone=$Zone lokasi=$Lokasi";
}
...
if ($sqlCek2 === false) {
	$sql_errors[] = "Gagal ambil count stokfull status belum cek untuk zone=$Zone lokasi=$Lokasi";
}
...
if ($_POST['cek'] == "Cek" or $_POST['cari'] == "Cari") {
	...
	// Pakai agregat supaya valid di SQL Server
	$sqlCek = sqlsrv_query_safe($con, "SELECT COUNT(*) as jml, MAX(zone) AS zone, MAX(lokasi) AS lokasi, MAX(kg) AS kg, MAX(lot) AS lot FROM dbnow_gdb.tbl_stokfull WHERE SN='$Barcode'");
	if ($sqlCek === false) {
		$sql_errors[] = "Gagal cek SN $Barcode di tbl_stokfull";
	}
	...
	if ($Zone == "" and $Lokasi == "") {
		...
		// Cek ke BALANCE, kalau lokasinya sama dengan input maka tandai OK juga
		$sqlDB21 = " SELECT WHSLOCATIONWAREHOUSEZONECODE, WAREHOUSELOCATIONCODE, CREATIONDATETIME, BASEPRIMARYQUANTITYUNIT, LOTCODE FROM 
		BALANCE b WHERE (b.ITEMTYPECODE='GYR' OR b.ITEMTYPECODE='DYR') AND b.LOGICALWAREHOUSECODE='M011' AND b.ELEMENTSCODE='$Barcode'AND WAREHOUSELOCATIONCODE = '$ck[lokasi]' AND WHSLOCATIONWAREHOUSEZONECODE = '$ck[zone]' ";
		$stmt1   = db2_exec($conn1, $sqlDB21, array('cursor' => DB2_SCROLLABLE));
		$rowdb21 = db2_fetch_assoc($stmt1);
		$lokasiBalance = $rowdb21 ? (trim($rowdb21['WHSLOCATIONWAREHOUSEZONECODE']) . "-" . trim($rowdb21['WAREHOUSELOCATIONCODE'])) : '';
		$tglMasuk = $rowdb21 ? substr($rowdb21['CREATIONDATETIME'], 0, 10) : '';
		$KGnow = $rowdb21 ? round($rowdb21['BASEPRIMARYQUANTITYUNIT'], 2) : 0;
		$lotNow = $rowdb21 ? sqlsrv_escape_str($rowdb21['LOTCODE']) : '';

		if (!$rowdb21) {
			// Data BALANCE ada tapi lokasi tidak sesuai -> catat ke stokloss
			...
			// Hanya bila data BALANCE ada dan lokasi sesuai input -> set OK
			sqlsrv_query_safe($con, "UPDATE dbnow_gdb.tbl_stokfull SET 
				  status='ok',
				  zone='$Zone',
				  lokasi='$Lokasi',
				  KG='$KGnow',
				  lot='$lotNow',
				  tgl_cek=GETDATE()
				  WHERE SN='$Barcode'", "CheckStock:update mismatch ok from balance");
		}
		...
		if ($ck['jml'] > 0) {
			$sqlData = sqlsrv_query_safe($con, "UPDATE dbnow_gdb.tbl_stokfull SET 
			  status='ok',
			  tgl_cek=GETDATE()
			  WHERE zone='$Zone' AND lokasi='$Lokasi' AND SN='$Barcode'", "CheckStock:update ok");
			$sqlCek1 = sqlsrv_query_safe($con, "SELECT COUNT(*) as jml FROM dbnow_gdb.tbl_stokfull WHERE status='ok' and zone='$Zone' AND lokasi='$Lokasi'", "CheckStock:recount ok");
			$ck1 = ($sqlCek1 !== false) ? sqlsrv_fetch_array($sqlCek1, SQLSRV_FETCH_ASSOC) : ['jml' => 0];
			$sqlCek2 = sqlsrv_query_safe($con, "SELECT COUNT(*) as jml FROM dbnow_gdb.tbl_stokfull WHERE status='belum cek' and zone='$Zone' AND lokasi='$Lokasi'", "CheckStock:recount belum");
			$ck2 = ($sqlCek2 !== false) ? sqlsrv_fetch_array($sqlCek2, SQLSRV_FETCH_ASSOC) : ['jml' => 0];
		} else {
			$sqlDB21 = " SELECT WHSLOCATIONWAREHOUSEZONECODE, WAREHOUSELOCATIONCODE, CREATIONDATETIME, BASEPRIMARYQUANTITYUNIT, LOTCODE FROM 
		BALANCE b WHERE (b.ITEMTYPECODE='GYR' OR b.ITEMTYPECODE='DYR') AND b.LOGICALWAREHOUSECODE='M011' AND b.ELEMENTSCODE='$Barcode' ";
			$stmt1   = db2_exec($conn1, $sqlDB21, array('cursor' => DB2_SCROLLABLE));
			$rowdb21 = db2_fetch_assoc($stmt1);
			$lokasiAsli = $rowdb21 ? (trim($rowdb21['WHSLOCATIONWAREHOUSEZONECODE']) . "-" . trim($rowdb21['WAREHOUSELOCATIONCODE'])) : '';
			$tglMasuk = $rowdb21 ? substr($rowdb21['CREATIONDATETIME'], 0, 10) : '';
			$KGnow = $rowdb21 ? round($rowdb21['BASEPRIMARYQUANTITYUNIT'], 2) : 0;
			$lotNow = $rowdb21 ? sqlsrv_escape_str($rowdb21['LOTCODE']) : '';

			$balanceMatch = $rowdb21 && $lokasiAsli === $Zone . "-" . $Lokasi;

			if ($balanceMatch) {
				// Lokasi Balance sama dengan input -> tandai OK di stokfull
				sqlsrv_query_safe($con, "UPDATE dbnow_gdb.tbl_stokfull SET 
					  status='ok',
					  zone='$Zone',
					  lokasi='$Lokasi',
					  ...
					  lot='$lotNow',
					  tgl_cek=GETDATE()
					  WHERE SN='$Barcode'", "CheckStock:update ok from balance");
				sqlsrv_query_safe($con, "IF NOT EXISTS (SELECT 1 FROM dbnow_gdb.tbl_stokfull WHERE SN='$Barcode')
					  INSERT INTO dbnow_gdb.tbl_stokfull (SN, zone, lokasi, KG, lot, tgl_masuk, status, tgl_cek)
					  VALUES ('$Barcode', '$Zone', '$Lokasi', '$KGnow', '$lotNow', '$tglMasuk', 'ok', GETDATE())", "CheckStock:insert ok from balance");
				$sqlCek1 = sqlsrv_query_safe($con, "SELECT COUNT(*) as jml FROM dbnow_gdb.tbl_stokfull WHERE status='ok' and zone='$Zone' AND lokasi='$Lokasi'", "CheckStock:recount ok");
				$ck1 = ($sqlCek1 !== false) ? sqlsrv_fetch_array($sqlCek1, SQLSRV_FETCH_ASSOC) : ['jml' => 0];
				$sqlCek2 = sqlsrv_query_safe($con, "SELECT COUNT(*) as jml FROM dbnow_gdb.tbl_stokfull WHERE status='belum cek' and zone='$Zone' AND lokasi='$Lokasi'", "CheckStock:recount belum");
				$ck2 = ($sqlCek2 !== false) ? sqlsrv_fetch_array($sqlCek2, SQLSRV_FETCH_ASSOC) : ['jml' => 0];
			} else {
				if ($rowdb21 && $lokasiAsli != "-") {
		...
		WHERE tu.status='Open' $Where ");
			...
			// Jika ada upload terbuka untuk SN ini, set ke ok juga
	...
	WHERE tu.status='Open' AND SN='$Barcode'");
			...
			if ($ck1['jml'] > 0) {
				$sqlData1 = sqlsrv_query_safe($con, "UPDATE dbnow_gdb.tbl_stokfull SET 
			  status='ok',
			  zone='$Zone',
			  lokasi='$Lokasi',
			  tgl_cek=GETDATE()
			  WHERE id_upload='" . $ck1['id_upload'] . "' AND SN='$Barcode'", "CheckStock:update ok by upload");
			}
		}
		/* 
else{
