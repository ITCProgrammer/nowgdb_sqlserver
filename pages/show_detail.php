...
include("../koneksi.php");
$modal_id = $_GET['id'];
$ORDERCODE = substr($modal_id, 0, 10);
$NOTRN = substr($modal_id, 11, 12);
$TGLTRN = substr($modal_id, 24, 10);
$kt = substr($modal_id, 35, 300);
$pos = strpos($kt, "-");
$ODERLINE = substr($kt, 0, $pos);
$LOT = substr($kt, $pos + 1, 300);
				...
				<table id="lookup1" class="table table-sm table-bordered table-hover table-striped" width="100%"
					...
					<thead>
						<tr>
							...
							<th>
								<div align="center">TANGGAL PRODUKSI</div>
							</th>
							<th>
					...
					<tbody>
						...
						$sqlDB22 = "SELECT 
							...
							RIGHT OUTER JOIN DB2ADMIN.BALANCE BALANCE ON 
								BALANCE.ELEMENTSCODE = STOCKTRANSACTION.ITEMELEMENTCODE  
							WHERE STOCKTRANSACTION.LOGICALWAREHOUSECODE='M011' 
							AND STOCKTRANSACTION.ORDERCODE='$ORDERCODE'
							AND STOCKTRANSACTION.ORDERLINE ='$ODERLINE' 
							AND STOCKTRANSACTION.TRANSACTIONNUMBER='$NOTRN' 
							-- AND STOCKTRANSACTION.LOTCODE='$LOT'
							";
						$stmt2 = db2_exec($conn1, $sqlDB22, array('cursor' => DB2_SCROLLABLE));
						while ($rD = db2_fetch_assoc($stmt2)) {

							 $sqlDB23 = "SELECT 
									ITTSUPPLIERPACKINGLISTBEAN.MANUFACTURINGDATE 
							 	FROM DB2ADMIN.ITTSUPPLIERPACKINGLISTBEAN ITTSUPPLIERPACKINGLISTBEAN
								WHERE ITTSUPPLIERPACKINGLISTBEAN.ITEMELEMENTCODE = '$rD[ELEMENTSCODE]'
							 ";
						
							 $stmt3 = db2_exec($conn1, $sqlDB23, array('cursor' => DB2_SCROLLABLE));
							 $rD2 = db2_fetch_assoc($stmt3);
							...
							echo "<tr'>
									...
									<td align=center>$rD2[MANUFACTURINGDATE]</td>
