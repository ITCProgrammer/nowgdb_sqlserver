...
function mutasiurut(){
	include "koneksi.php";		
	$format = "20".date("ymd");
	$sql=sqlsrv_query($con,"SELECT TOP 1 no_mutasi FROM dbnow_gdb.tbl_mutasi_kain WHERE SUBSTRING(no_mutasi,1,8) like '%".$format."%' ORDER BY no_mutasi DESC");
	if($sql === false){
...
$sql1=sqlsrv_query($con,"SELECT *,count(b.transid) as jmlrol,a.transid as kdtrans FROM dbnow_gdb.tbl_mutasi_kain a 
LEFT JOIN dbnow_gdb.tbl_prodemand b ON a.transid=b.transid 
WHERE a.no_mutasi IS NULL AND CONVERT(varchar(10),a.tgl_buat,120)='$Awal' AND a.gshift='$Gshift' 
GROUP BY a.transid");
$n1=1;
$noceklist1=1;	
while($r1=sqlsrv_fetch_array($sql1, SQLSRV_FETCH_ASSOC)){	
	if($_POST['cek'][$n1]!='') 
		...
		sqlsrv_query($con,"UPDATE dbnow_gdb.tbl_mutasi_kain SET
		no_mutasi='$nomid',
		tgl_mutasi=GETDATE()
