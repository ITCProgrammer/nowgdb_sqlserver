...
WHERE
	m.CHALLANDATE BETWEEN '$_GET[tgl1]' AND '$_GET[tgl2]'
...
ORDER BY
	x.TRANSACTIONNUMBER ASC ";
          ...
          foreach ($dataMain as $r) {
            ...
            if ($r['DESTINATIONWAREHOUSECODE'] == 'M904') {
              $knitt = 'KNITTING ITTI ATAS- BENANG';
            } else if ($r['DESTINATIONWAREHOUSECODE'] = 'P501') {
              $knitt = 'KNITTING ITTI- BENANG';
            } else if ($r['DESTINATIONWAREHOUSECODE'] = 'M051') {
              $knitt =  'KNITTING A- BENANG';
            } else if ($r['DESTINATIONWAREHOUSECODE'] = 'P503') {
              $knitt =  'YARN DYE';
            }
            $brt = number_format(round($r['QTY_KG'], 2), 2);
            if ($r['USERSECONDARYUOMCODE'] == "BL") {
              $stn = "KARUNG";
            } else {
              $stn = "DUS";
            }
	...
	<td>$r[LOKASI]</td>
            ...
            if ($stn == 'DUS') {
              $dustot = $dustot + $r['QTY_DUS'];
              $totdus = $totdus + $r['QTY_KG'];
            };
            if ($stn == 'KARUNG') {
              $kartot = $kartot + $r['QTY_DUS'];
              $totkar = $totkar + $r['QTY_KG'];
            }
            if ($stn == 'PALET') {
              $pltot = $pltot + $r['QTY_DUS'];
              $totpl = $totpl + $r['QTY_KG'];
            }
            if ($stn == 'CONES') {
              $contot = $contot + $r['QTY_DUS'];
              $totcon = $totcon + $r['QTY_KG'];
            }
            $totr = $totr + $r['QTY_DUS'];
            $no++;
          }
